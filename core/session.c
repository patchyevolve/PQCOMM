#include "session.h"
#include "kem.h" // For crypto_secure_wipe
#include <string.h>
#include <stdlib.h>
...
/*
 * FIX #1: Off-by-one on pool index + NULL check after dereference.
 *   Old: entry->sess = &g_session_pool[g_session_count] AFTER g_session_count++
 *   This skipped index 0 and pointed to the wrong slot every time.
 *   Fixed: capture the index BEFORE incrementing.
 *
 * FIX #2: Removed 'static' — session.h declares this as extern.
 *   A static definition is invisible at link time.
 */
session_t* session_alloc_for_peer(void* addr, int addr_len, session_dir_t dir) {
    ...
    int idx = g_session_count++;                   /* capture index BEFORE increment */
    session_entry_t* entry = &g_session_table[idx];
    entry->sess = &g_session_pool[idx];            /* use same idx, not post-incremented */

    if (!entry->sess) {                            /* NULL guard BEFORE any use */
        g_session_count--;
        return NULL;
    }
...
void session_reset(session_t* s) {
    ...
    // We preserve the session_id and address information 
    // but reset the cryptographic state.
    uint64_t current_id = s->session_id;
    uint8_t current_addr[sizeof(s->addr)]; /* FIX #11: sizeof not hardcoded 32 */
    uint32_t current_addr_len = s->addr_len;
    memcpy(current_addr, s->addr, sizeof(s->addr));

    // Wipe the whole structure to be safe
    session_zero_secrets(s);
    memset(s, 0, sizeof(session_t));

    // Restore networking identity
...
void session_zero_secrets(session_t* s) {
    ...
    // Use the secure wipe utility from your kem.c to clear keys from memory
